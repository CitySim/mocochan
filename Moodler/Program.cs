	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			if (Program.config.writeInfo)
			{
				...
				Console.WriteLine("Moodler CLI v. {0}", version);
				Console.WriteLine();
			}
			...
			if (Program.config.writeHelp)
			{
				...
				Console.WriteLine(" Moodler -e [options] inputFile.ext");
				Console.WriteLine();
			...
			Converter Moodler = new Converter(settings, new ConsoleLogProvider());
			Moodler.loadPlugins(Program.config.PluginDirectory);

			foreach (string file in Program.config.InputFiles)
			{
				Moodler.Convert(file);
			}
